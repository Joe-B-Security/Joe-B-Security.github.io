...
Every step here is individually correct. The support agent is supposed to delegate when work exceeds its permissions, the ops agent is supposed to query the database and document its findings, and the support agent is supposed to read results and reply to the ticket submitter. The vulnerability exists in the composition, in how sensitive data flows from a high-privilege context through a shared resource into a customer-facing reply. This can happen whether or not an attacker is involved.
...
SAST sees individual functions, so it can find SQL injection in a query builder or XSS in a template renderer. But it cannot see that data returned by `query_customer_db` in one agent's context flows through `write_results`, into a shared store, back through `read_results` in a different agent's context, and out through `reply_to_customer` to an end user. The vulnerability spans multiple agents, multiple tool calls, and a shared resource, so there is no single function to flag.

DAST tests endpoints and responses, so it can find misconfigurations, missing headers, and exposed debug pages. But it cannot test the internal delegation chain between agents because there is no endpoint for it. The delegation is an internal workflow that produces a customer-visible effect only when the full chain executes in a specific way.

RBAC asks "does this agent have the role required to call this tool?" The support agent has the support role. The support role can call `read_results`. Access granted. You can add resource-level scoping to RBAC (restricting `read_results` to specific store partitions), and that would also block this particular leak, but those grants need to be created and maintained per task, which does not scale well when tasks are created dynamically by agent delegation. RBAC authorises based on the caller's role, not on the relationship between the caller's task and the specific resource.

The question that matters here is whether data from a high-privilege context can reach a context where it gets published externally. That is a graph reachability question across agents, tasks, resources, and tools, and it is not the kind of question that SAST, DAST, or RBAC are designed to answer.

## Defence in depth for agent composition

Authorisation bounds which data can reach which context, but it is one layer of a broader defence. Data classification at write time, sensitivity-aware summarisation for cross-context handoffs, and output filtering at response boundaries all matter too. The rest of this post focuses on the authorisation layer, specifically relationship-based access control and why it fits agent composition better than role-based access control. The [final section](#what-authorisation-does-not-solve) covers where architectural fixes take over.
...
Where RBAC asks "does this user have a role that includes this permission?", Zanzibar asks "does this user have a relationship to this specific resource?" The first depends on the user's role, which is a property of the user. The second depends on the graph structure, which can encode context that roles cannot: which task the user is working on, who delegated the work, what resources were created during the task, and how those resources relate to other tasks.
...
The tuples encode things that RBAC cannot easily express. The delegation chain is explicit and traceable, so if something goes wrong you can see which agent was assigned, which task created the delegation, and what resources each task could access.

Resource access is scoped to tasks rather than agent identities, so the ops agent working on a different task would have different resource grants.

Delegation does not inherit permissions. TASK-001 delegated work to TASK-002, but that link does not give TASK-001 access to TASK-002's resources. The parent can hand off work, but it cannot read back the child's outputs unless someone explicitly grants that access.

With this model in place, the same scenario plays out differently. Steps 1 through 4 pass their permission checks normally. The interesting moment is step 5, when the support agent tries to read the ops agent's findings. Instead of checking whether the agent's role allows calling `read_results`, the engine checks the `reader` relation against the specific resource: does TASK-001 have `reader` access to `results_store:TASK-002`?
...
ReBAC constrains which data can reach which context, but it is one layer of a broader secure design. In most real systems, data does need to flow back across task boundaries, and when it does the architectural choices matter just as much as the authorisation model. Data classification at write time lets downstream systems distinguish SSNs from refund recommendations. Sensitivity-aware summarisation can produce redacted versions of findings for cross-context consumption. Output filtering at response boundaries can catch PII before it leaves the system regardless of what the agent decided to include. These are complementary concerns, and they work best when they are part of the original architecture rather than added later.
...
The hardest part of adopting relationship-based access control for agent systems is the cold start, figuring out what relationships to model, what the delegation boundaries should be, and where the resource-level checks need to go. The model in this demo is small enough to read in a few minutes and maps directly to production Zanzibar implementations. I found it useful for building intuition about how graph-based authorisation applies to agent composition.
